...
12. 自建运放子电路时必须限制输出摆幅（E 源输出用 min/max 钳位到电源轨：
    E 源写 VALUE={max(-Vs, min(1e5*v(a,b), +Vs))}，Vs=电源电压-0.5），
    否则理想增益链会把输出推到 1e10V 以上导致数值发散、全电路指标作废。
    注意：ngspice 的 limit() 实测不做钳位（静默返回错误值），禁用它。
    运放模型内部用 E 源是标准做法，不算行为源作弊。
"""
